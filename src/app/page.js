...
function Starfield(){const c=useRef(null);useEffect(()=>{const el=c.current;if(!el)return;const ctx=el.getContext("2d",{alpha:!1});let w=el.width=window.innerWidth,h=el.height=window.innerHeight;const dpr=Math.min(2,window.devicePixelRatio||1);el.width=w*dpr;el.height=h*dpr;ctx.scale(dpr,dpr);const N=Math.min(240,Math.floor(w*h/11000));const stars=Array.from({length:N},()=>({x:Math.random()*w,y:Math.random()*h,vx:(Math.random()-.5)*.12,vy:(Math.random()-.5)*.12,r:.7+Math.random()*1.3,a:.5+.5*Math.random()}));let raf;const loop=()=>{ctx.fillStyle="#0b0f1a";ctx.fillRect(0,0,w,h);for(const s of stars){s.x+=s.vx;s.y+=s.vy;if(s.x<-2)s.x=w+2;if(s.x>w+2)s.x=-2;if(s.y<-2)s.y=h+2;if(s.y>h+2)s.y=-2;ctx.beginPath();ctx.fillStyle=`rgba(124,249,255,${s.a})`;ctx.arc(s.x,s.y,s.r,0,Math.PI*2);ctx.fill()}raf=requestAnimationFrame(loop)};raf=requestAnimationFrame(loop);const onR=()=>{w=el.width=window.innerWidth;h=el.height=window.innerHeight;el.width=w*dpr;el.height=h*dpr;ctx.scale(dpr,dpr)};window.addEventListener("resize",onR);return()=>{cancelAnimationFrame(raf);window.removeEventListener("resize",onR)}},[]);return <canvas ref={c} className="fixed inset-0 -z-10"/>}
...
function Porthole({item, open, onToggle}){
  return (
    <div className="relative flex flex-col items-center w-[168px] md:w-[200px]">
      <button
        ...
        {/* title overlay fades and becomes invisible when open */}
        <motion.div
          initial={false}
          animate={{opacity: open?0:1}}
          transition={{duration:.25}}
          className={`absolute inset-0 z-20 flex items-center justify-center text-center px-3
        ...
        <motion.div
          className="absolute inset-0 z-30 bg-[linear-gradient(180deg,rgba(10,14,24,.88),rgba(10,14,24,.2))]"
          initial={{y:0}}
          animate={{y: open ? "-100%" : "0%"}}
          transition={{duration:.42, ease:[.22,.9,.24,1]}}
        />
        <motion.div
...
function Row({items,active,onToggle}){
  return (
    <div className="flex items-center gap-4 md:gap-6 w-full">
      {items.map((it,i)=>(
        <div key={it.id} className="flex items-center gap-4 md:gap-6 flex-1">
          <Porthole item={it} open={active===it.id} onToggle={onToggle}/>
          ...
          {tab==="experience" && (
            <>
              ...
              <div className="space-y-8">
                <Row items={exp.top}    active={open} onToggle={setOpen}/>
                <Row items={exp.bottom} active={open} onToggle={setOpen}/>
              </div>
                ...
                {details && (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <button onClick={()=>setOpen(null)} className="text-sm px-3 py-1.5 rounded-lg ring-1 ring-white/25 bg-white/10 hover:bg-white/15">
                        Close
                      </button>
                    </div>
